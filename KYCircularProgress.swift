...
class KYCircularProgress: UIView {
    ...
    var lineWidth: Double = 8.0 {
        willSet {
            ...
            self.progressView.shapeLayer().lineCap = kCALineCapRound
        }
    }
    ...
    fileprivate func setup() {
        ...
        gradientLayer.colors = self.colors ?? [colorHex(0xffffff).cgColor, colorHex(0xffffff).cgColor]
    ...
    fileprivate func updateColors(_ colors: [Int]?) -> () {
        ...
        if let inputColors = self.colors {
            ...
            convertedColors = [colorHex(0x9ACDE7).cgColor, colorHex(0xE7A5C9).cgColor]
        }
